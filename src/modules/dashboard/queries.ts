...
export async function getDashboardData() {
  ...
  const productionByProductResult = await db.execute(sql`
      SELECT
         p.id as "productId",
         ...
         coalesce((SELECT sum(po.target_qty) FROM production_orders po
                   WHERE po.product_id = p.id AND po.status IN ('in_progress', 'completed')
                     AND (po.created_at AT TIME ZONE 'UTC')::date >= ${monthStart}::date), 0) as "monthTarget",
       ...
       WHERE p.active = true
         AND (
           ...
           OR EXISTS (SELECT 1 FROM production_orders po WHERE po.product_id = p.id
                        ...
                        AND (po.created_at AT TIME ZONE 'UTC')::date >= ${monthStart}::date)
         )
       ORDER BY "monthQty" DESC, p.name ASC
  `);
  const productionByProduct = productionByProductResult.rows as unknown as {
    productId: number;
    productName: string;
    uom: string | null;
    todayQty: number;
  ...
  const productionTrendResult = await db.execute(sql`
      SELECT b.product_id as "productId", p.name as "productName", b.uom as "uom",
       ...
       GROUP BY b.product_id, p.name, b.uom, b.mfg_date
       ORDER BY b.mfg_date ASC
  `);
  const productionTrend = productionTrendResult.rows as unknown as {
    productId: number;
    productName: string;
    uom: string;
    date: string;
  ...
  const activeOrdersResult = await db.execute(sql`
      SELECT po.id, po.order_no as "orderNo", p.name as "productName", po.target_qty as "targetQty",
       ...
       ORDER BY po.id DESC
  `);
  const activeOrders = activeOrdersResult.rows as unknown as {
    id: number;
    orderNo: string;
    productName: string;
    targetQty: number;
  ...
  const lowStockResult = await db.execute(sql`
      SELECT i.id as "itemId", i.name as "itemName", i.uom, i.reorder_level as "reorderLevel",
       ...
       WHERE i.active = true AND i.reorder_level > 0
       GROUP BY i.id
       HAVING coalesce(sum(sb.qty), 0) <= i.reorder_level
       ORDER BY coalesce(sum(sb.qty), 0) / i.reorder_level
  `);
  const lowStock = lowStockResult.rows as unknown as { itemId: number; itemName: string; uom: string; reorderLevel: number; qty: number }[];

  const recentBatchesResult = await db.execute(sql`
  ...
  `);
  const recentBatches = recentBatchesResult.rows as unknown as {
    id: number;
    batchNo: string;
    productName: string;
    qtyProduced: number;
  ...
  const pendingInspectionsResult = await db.execute(sql`SELECT count(*) as n FROM lots WHERE qc_status = 'pending'`);
  const batchesAwaitingQCResult = await db.execute(
    sql`SELECT count(*) as n FROM batches WHERE qc_status IN ('pending', 'sample_collected', 'testing')`
  );
  const batchesOnHoldResult = await db.execute(sql`SELECT count(*) as n FROM batches WHERE qc_status = 'hold'`);
  const openCapasResult = await db.execute(sql`SELECT count(*) as n FROM capas WHERE status != 'closed'`);
  ...
  return {
    ...
    expiring,
    recentBatches,
    pendingInspections: Number((pendingInspectionsResult.rows[0] as { n: number | string }).n),
    batchesAwaitingQC: Number((batchesAwaitingQCResult.rows[0] as { n: number | string }).n),
    batchesOnHold: Number((batchesOnHoldResult.rows[0] as { n: number | string }).n),
    openCapas: Number((openCapasResult.rows[0] as { n: number | string }).n),
    maintenanceAlerts,
  };
}
