...
// extra table, can't drift. Accepts an optional `tx` so it can be called
// both standalone and from inside saveLayoutEdits's atomic() block, reading
// a consistent snapshot in the latter case.
export async function computeLayoutVersion(tx: DbOrTx = db): Promise<string> {
  const bedResult = await tx.execute(
    sql`SELECT id, zone_id, code, x1, y1, x2, y2, width_ft, active FROM beds ORDER BY id`
  );
  const featureResult = await tx.execute(
    sql`SELECT id, kind, structure_type, label, polygon, warehouse_id, active FROM site_features ORDER BY id`
  );
  const zoneResult = await tx.execute(sql`SELECT id, code, polygon, label_x, label_y FROM zones ORDER BY id`);
  return createHash("sha1")
    .update(JSON.stringify([bedResult.rows, featureResult.rows, zoneResult.rows]))
    .digest("hex")
    .slice(0, 16);
}

export async function getBedLayout() {
  const zoneRows = await db.select().from(zones).orderBy(asc(zones.code));
  ...
  const bedRows = await db.select().from(beds).where(eq(beds.active, true)).orderBy(asc(beds.code));
  ...
  `);
  const occupancy = occupancyResult.rows as unknown as OccupancyRow[];
  ...
  return {
    ...
    version: await computeLayoutVersion(),
  };
}

export type BedLayout = Awaited<ReturnType<typeof getBedLayout>>;
export type LayoutBed = BedLayout["beds"][number];

// Beds that are free (no in-progress order), for assignment pickers.
export async function getBedsWithAvailability(excludeOrderId?: number) {
...
export async function getOrderBedIds(orderId: number): Promise<number[]> {
  return (await db.select({ bedId: orderBeds.bedId }).from(orderBeds).where(eq(orderBeds.orderId, orderId))).map(
    (r) => r.bedId
  );
}
